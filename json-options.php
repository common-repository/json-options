...
class jsonOptions {
    ...
    $this->fields = array(
      ...
      array(
		...
		'default' => array( 'janrain_capture', 'rpx' ),
      	'selected' => 'disabled',
		'options' => get_option( self::$name . '_filters_available', array( 'janrain_capture', 'rpx' ) ),
		'screen' => 'main',
	   ),
       array(
   ...
   */
  function activate() {
  	if ( class_exists( 'JanrainCapture' ) ) { return false; }
    foreach ( $this->fields as $field ) {
        if ( $field['name'] == self::$name.'_filter_remove' &&
        		get_option( self::$name.'_filters_available' ) === false) {
        	update_option( self::$name.'_filters_available', $field['default'] );
    	}
     }
  }
  /**
   ...
   */
  function admin_menu() {
  	// if we are in debug mode AND have debug mode disabled
  	if ( class_exists( 'JanrainCapture' ) && WP_DEBUG == false) {
  		// clean/simple janrain capture only fold-in
  		$optPage = add_submenu_page(
		    	'janrain_capture', __( 'Janrain Capture - Export or Import Settings' ), __( 'Export/Import Settings' ),
		    	'manage_options', self::$name, array( &$this, 'clean' )
		);
  	} else {
  		// stand-alone menu
  ...
  /**
   * Method bound to the JSON Options clean menu.
   */
  function clean() {
  	$args = new stdClass;
    $args->title  = 'Export or Import Janrain Capture Settings';
    $args->action = 'clean';
...
CHECKBOX;
      ...
      case 'hidden':
        echo <<<HIDDEN
            <input type='hidden' name="{$field['name']}" value="{$field['default']}"></input>
HIDDEN;
        break;
    }
  }
  ...
  public function on_post() {
  	...
  	// importing
  	if( $_FILES && $_FILES[self::$name .'_upload'] && $_FILES[self::$name .'_upload']['name'] != '' ) {
  		$this->json_import();
  	// exporting
  	} elseif ( isset($_POST[self::$name . '_action']) ) {
  	...
  	// managing filters
  		...
  		if ( isset ( $_POST[self::$name . '_filter_add']) && $_POST[self::$name . '_filter_add'] != '' ) {
  			...
  			} elseif ( in_array( $filter, $this->filters ) ) {
  				$this->postMessage = array( 'class' => 'error', 'message' => "Filter Already Exists" );
  			} else {
  ...
  private function save_fields() {
    ...
    private function json_import() {
    	...
    	if ( $_FILES[self::$name . "_upload"]["error"] > 0 ) {
    		$this->postMessage = array( 'class' => 'error', 'message' => 'Import Failed - ' . $_FILES[self::$name . "_upload"]["error"] );
	 	} else {
    	...
    	$changed = count( array_filter( $this->queries ) );
    	if ( $this->queries != null && $changed > 0  ) {
   			$this->postMessage = array( 'class' => 'updated', 'message' => "Import Success - $changed/$count (updated/input)" );
   		} elseif ( $this->queries == null ) {
   			$this->postMessage = array( 'class' => 'error', 'message' => "Nothing to Import - Check Filters" );
   		} elseif ( $changed == 0 ) {
   			$this->postMessage = array( 'class' => 'error', 'message' => "Nothing to Import - No Changes" );
   		} else {
   			$this->postMessage = array( 'class' => 'error', 'message' => 'Import Failed');
   		}
  	...
  	private function json_export() {
        ...
        unset($arr['janrain_capture_client_secret'], $arr['janrain_capture_widget_client_secret'], $arr['rpx_api_key_option']);
     ...
     * For compatability with PHP versions < 5.4
	 *
	 * @author dave perrett
